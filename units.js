/**
 * available selectable currencies for the application
 * https://gist.github.com/stevekinney/8334552
 */
...
currency_selector.addEventListener("change", (e) => {
  e.preventDefault();
  let currency_symbols = document.querySelectorAll(
    "span[class='currency-symbol']"
  );
  currency_symbols.forEach((symbol) => {
    symbol.innerText = currency_selector.value;
  });
});
/**
 * populates the currency selector with the available units
 *
 * @param {String} default_currency default selected currency
 */
const populate_currency_selector = (default_currency) => {
  currencies.forEach((c) => {
    let option = document.createElement("option");
    option.setAttribute("value", c.symbol);

    if (c.abbreviation === default_currency) {
      option.setAttribute("selected", true);
    }

    option.appendChild(
      document.createTextNode(`${c.abbreviation}: ${c.currency}`)
    );
    currency_selector.appendChild(option);
  });

  // triggering the event listener to populate the text
  currency_selector.dispatchEvent(new Event("change"));
};
populate_currency_selector("USD");

/**
 * available units of measurement for the pizza
 */
export const units = [
  {
    unit: "inch",
...
export const unit_selector = document.getElementById("unit_selector");
unit_selector.addEventListener("change", (e) => {
  e.preventDefault();
  let unit_symbols = document.querySelectorAll("span[class='unit-symbol']");
  unit_symbols.forEach((symbol) => {
    symbol.innerText = unit_selector.value;
  });
});
/**
 * populates the unit selector with the available units
 *
 * @param {String} default_unit default selected unit
 */
const populate_unit_selector = (default_unit) => {
  units.forEach((u) => {
    let option = document.createElement("option");
    option.setAttribute("value", u.abbreviation);

    if (u.abbreviation === default_unit) {
      option.setAttribute("selected", true);
    }

    option.appendChild(document.createTextNode(`${u.abbreviation}: ${u.unit}`));
    unit_selector.appendChild(option);
  });

  // triggering the event listener to populate the text
  unit_selector.dispatchEvent(new Event("change"));
};
populate_unit_selector("in");
 ...
 */
export const convert_in_to_cm = (inches) => {
  return Math.round(100 * inches * centimeter_per_inch) / 100;
};

/**
 * converts centimeters into inches
 *
 * @param {Number} inches number of centimeters
 * @returns {Number} inches
 */
export const convert_cm_to_in = (centimeters) => {
  return Math.round((100 * centimeters) / centimeter_per_inch) / 100;
};
